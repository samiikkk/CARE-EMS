...
def measure_inference(model, loader, device, n_warmup_batches=5):
    ...
    avg_latency_ms = np.mean(latencies) * 1_000   # ms per sample


    return avg_latency_ms, total_samples


def print_profiling_report(model, test_loader, device, peak_mem_mb):
    ...
    print(f"GPU peak memory: {peak_mem_mb:.2f} MB", flush=True)
